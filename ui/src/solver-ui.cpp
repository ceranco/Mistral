...
void register_output_callback(void (*write_fn)(string)) { _display = write_fn; }

void display(string s) { _display(s); }
...
class Term;
class Variable;
AccessPath *constraint_to_ap(CNode *c, IndexVarManager &ivm) { assert(false); }

CNode *ap_to_constraint(AccessPath *ap) { assert(false); }

void replace_access_path(AccessPath *ap_old, AccessPath *ap_new) {
  assert(false);
}

Term *make_term(AccessPath *ap_old) { assert(false); }

AccessPath *make_ap_from_term(Term *t, IndexVarManager &ivm,
                              set<AccessPath *> &to_add,
                              map<Term *, Variable *> &replacement_map) {
  assert(false);
}
...
Variable *Variable::clone() { assert(false); }
} // namespace sail
 ...
 * Global variables
 */
string levels[] = {"Solve only", "Solve only (DNF)", "Boolean Simplify",
                   "Hybrid Simplify", "Theory Simplify"};
...
string get_line(string s, int line, bool include_later_lines = false) {
  ...
  while (line >= 0 && i < s.size()) {
    ...
    while (i < s.size() &&
           ((line == 0 && include_later_lines) || s[i] != '\n')) {
      first_line += s[i++];
    }
    i++;
    line--;
...
bool is_solve_regression(string s) {
  ...
  if (first_line.find("solve") != string::npos)
    return true;
  return false;
}
...
void add_solve_regression(string file, string regression) {
  ...
  bool sat;
  if (status_line.find("UNSAT") != string::npos)
    sat = false;
  else if (status_line.find("SAT") != string::npos)
    sat = true;
  else {
...
void fill_solve_regressions(set<string> &files) {
  ...
  for (; it != files.end(); it++) {
    ...
    if (!is_solve_regression(regression))
      continue;
...
void add_regresssion_button_clicked() {
  ...
  while (true) {
    ...
    if (solve_regressions.count(id) == 0)
      break;
  }
  string file;
  file = "Type: solve\n";
  ...
  if (!myfile.is_open()) {
    Gtk::MessageDialog dialog(
        "Regression folder \"" + dir + "\" does not exist. Create?", false,
...
void print_error(string s) { cout << "Error: " << s << endl; }
  ...
  */
  ...
  if (argc >= 2 && strcmp(argv[1], "--regress") == 0) {
    ...
    for (; it != solve_regressions.end(); it++) {
      ...
      if ((status == SAT && it->second.status) ||
          (status == UNSAT && it->second.status == false)) {
        // success
      }
  ...
  if (argc >= 3 && strcmp(argv[1], "--file") == 0) {
    ...
    for (int i = 0; i < 1; i++) {
      ...
      sat_query_status status =
          run_mistral(constraint, ticks, false, true, size);
  ...
  if (argc >= 3 && strcmp(argv[1], "--smtlibfolder") == 0) {
    ...
    for (set<string>::iterator it = files.begin(); it != files.end(); it++) {
      ...
      if (p_id == 0) {
        ...
        cout << "*****File: " << *it << " SAT " << ticks_to_seconds(end)
             << endl;
        ...
        cout << "#File: " << *it << " Solve time: " << ticks_to_seconds(end)
             << " MSA time: " << ticks_to_seconds(msa_time)
             << " #vars: " << num_vars << " MSA size: " << msa_size << endl;
  ...
  display_changed->connect(sigc::ptr_fun(refresh_display));

  solve_status = new Glib::Dispatcher();
  solve_status->connect(sigc::ptr_fun(solve_completed));
...
void assign_changed() { assign = assign_button->get_active(); }

void no_verify_changed() {
  verify_simplification = !no_verify_button->get_active();
}

void print_stats_changed() { print_stats = print_stats_button->get_active(); }
...
void init_solve_elements(Glib::RefPtr<Gnome::Glade::Xml> &refXml) {
  ...
  cancel_button->signal_clicked().connect(sigc::ptr_fun(cancel_button_clicked));
  ...
  input_buffer->signal_changed().connect(sigc::ptr_fun(input_buffer_changed));
  ...
  elim_button->signal_clicked().connect(sigc::ptr_fun(elim_button_clicked));
  ...
  assign_button->signal_clicked().connect(sigc::ptr_fun(assign_changed));
  assign_changed();

  refXml->get_widget("no_verify_checkbutton", no_verify_button);
  assert(no_verify_button != NULL);
  no_verify_button->signal_clicked().connect(sigc::ptr_fun(no_verify_changed));
  no_verify_changed();
  ...
  level_combobox->signal_changed().connect(sigc::ptr_fun(level_changed));
  level_changed();
...
void refresh_solve_regression_info() {
  ...
  double fraction =
      ((double)num_success + num_failed) / ((double)solve_regressions.size());
  ...
  if (regressions_finished) {
    if (regressions_canceled)
      ...
      regression_status->set_text("Status: Completed");
...
void init_regression_elements(Glib::RefPtr<Gnome::Glade::Xml> &refXml) {
  ...
  only_failed->signal_clicked().connect(sigc::ptr_fun(update_regress_treeview));

  refXml->get_widget("no_filter", show_all);
  assert(show_all != NULL);
  show_all->signal_clicked().connect(sigc::ptr_fun(update_regress_treeview));
...
void init_msa_elements(Glib::RefPtr<Gnome::Glade::Xml> &refXml) {
  ...
  msa_button->signal_clicked().connect(sigc::ptr_fun(run_msa));
...
void init_opt_elements(Glib::RefPtr<Gnome::Glade::Xml> &refXml) {
  ...
  opt_button->signal_clicked().connect(sigc::ptr_fun(run_opt));
...
void solve_button_clicked() {
  ...
  int ticks;
  int size;
  run_mistral(input_str, ticks, !single_process_button->get_active(), false,
              size);
}
...
void timeout_changed() { timeout = timeout_button->get_value_as_int(); }

void select_regression_changed() {
  int num = select_regression_button->get_value_as_int();
  if (num < 0 || num >= (int)solve_regressions.size())
    return;
  map<string, solve_regression>::iterator it = solve_regressions.begin();
  while (num > 0) {
    num--;
 ...
 */
bool check_command_line(int argc, char **argv) { return false; }
...
string get_display_string(sat_query_status status, int ticks, Solver &s,
  ...
  if (status == VERIFY_ERROR) {
    return "Error: Result constraint " + s.get_result()->to_string() +
  ...
  string res =
      "Total time (s): " + float_to_string(ticks_to_seconds(ticks)) + "\n";
  if (print_stats)
    res += s.get_stats();
  if (status == UNSAT)
    res += "UNSAT";
  ...
  if(s2.get_result()->get_type() != FALSE_NODE) {
          ...
          //for(int i=0; i<1;i++) {
                  ...
                  if(cost==-1) cost = new_cost;
                          ...
                          if(new_cost != cost) {
                                  cout << "cost is: " << cost << "  new cost is:
  " << new_cost << endl; cout << "wrong costs: "<<
  replace_funs_with_vars(node)->to_string() << endl; assert(false);
                          }
                  }
          ...
          string r =  float_to_string(((double)time)/((double)CLOCKS_PER_SEC)) +
                          "\t" + int_to_string(vt.size()) + "\t" +
  int_to_string(node->get_size())
                          + "\t"+int_to_string(cost) + "\t"
  +int_to_string(msa_f.mpi_estimate)+
                          "\t" +
  ...
  */
  ...
  /*{
          ...
          if(not_res != ss.get_result())
          {
                  ...
                  cout << "Second simp: " << ss.get_result()->to_string() <<
  endl;
          }
  }*/
  ...
  if (info != NULL)
    *info = get_display_string(status, time, s, print_stats, mm);
  return status;
}
...
bool is_sat(CNode *constraint) {
  ...
  if (constraint->get_type() == FALSE_NODE)
    return false;
  if (constraint->get_type() == TRUE_NODE)
    return true;
...
AccessPath* make_ap_from_term(Term* t, IndexVarManager& ivm,
                set<AccessPath*>& to_add, map<Term*, Variable*>&
replacement_map)
{
        assert(false);
}
...
bool is_satisfiable_ui(CNode* constraint, map<Term*,SatValue>* sat_assignments =
NULL, bool verbose = false, bool ilp_only =false) {
...
pair<CNode*, CNode*> eliminate_evars(CNode* c, bool regress = false)
{
        ...
        if(evars.size() != 0)
        {
                ...
                const map<Term*, CNode*> & eqs =
ef.get_disjunctive_equalities(); display_out("EQUALITIES::\n"); map<Term*,
CNode*>::const_iterator it = eqs.begin(); for(; it != eqs.end(); it++) {
                        display_out ("Equality: " + it->first->to_string() + "
under "+ it->second->to_string()); display_out("\n");
                ...
                bool new_process = true)
{
        ...
        if(!new_process)
        {
                ...
                if(verify_simplification) {
                        bool equivalent = constraint_equivalent(node,
s.get_result()); if(!equivalent){ cout << "ERROR: NOT EQUIVALENT:" << endl;
        ...
        if(p_id == 0)
        {
...

                {
                        ...
                        if(verify_simplification) {
                                bool equivalent = constraint_equivalent(node,
s.get_result()); if(!equivalent){ v = 2; cout << "ERROR: NOT EQUIVALENT:" <<
endl; display_out("ERROR: NOT EQUIVALENT:");
                                }
                        }
...
+ ":\n"); display_eregress(r.sc + "\n"); continue;
                ...
                if(!constraint_equivalent(nc_node, res.first)){
                        success = false;
                        display_eregress("Regression " + int_to_string(i) + "
failed: \n"); display_eregress("NC not equivalent: Expected: " + r.nc + "
Result: " + res.first->to_string() +"\n"); cur_succ = false;
                }

                if(!constraint_equivalent(sc_node, res.second)){
                        success = false;
                        display_eregress("Regression " + int_to_string(i) + "
failed: \n"); display_eregress("SC not equivalent: Expected: " + r.sc + "
Result: " + res.second->to_string()+"\n" ); cur_succ = false;
                }
...
void run_regressions()
{
         ...
         for(unsigned int i=0; i < regs.size(); i++)
         {
                ...
                else
                {
                        ...
                        sat_query_status s = run_mistral(constraint, cur_ticks,
true);
...
\n"); success = false;
                        ...
                        else
                        {
                                ...
                                if(sat != r.sat)
                                {
                                        ...
                                        if(sat)
                                                display_regress("SAT instead of
UNSAT\n"); else display_regress("UNSAT instead of SAT\n");
                ...
                if(!success){
                        display_regress("Regression " + int_to_string(i) + "
failed:\n");
                }
                else {
                        display_regress("Regression " + int_to_string(i) + "
passed: "
                                +
float_to_string((double)cur_ticks/(double)CLOCKS_PER_SEC)+ "s \n");
                }
                if(!success)
                        total_success = false;
         }
...
int main(int argc, char** argv) {
        ...
        if(argc >=2) {
                ...
                if(strcmp(argv[1], "--file") == 0) {
                        ...
                        cout << "Time: " <<
((double)time)/((double)CLOCKS_PER_SEC) << endl;
                }
                else if(strcmp(argv[1], "--regress") == 0)
                {
                        ...
                        for(unsigned int i=0; i < regs.size(); i++)
                        {
                                ...
                                Regression &r = regs[i];
                                CNode::clear();
                                CNode* constraint = parse_constraint(r.query,
NULL); if(constraint ==  NULL) { cout << "Regression # " << i << " does not
parse." << endl; success = false; continue;
                                }
                                int start = clock();
                                ...
                                ticks+=(end-start);
                                bool sat =
s.get_result()->get_type()!=FALSE_NODE; if(sat!=r.sat){ cout << "Regression #"
<< i << " failed." << endl; success = false;
                                }
                                else
                                        cout << "Success (" <<
...
<< ") seconds"
        ...
        Gtk::Window* ui_window = NULL;
        Glib::RefPtr<Gnome::Glade::Xml> refXml =
                                Gnome::Glade::Xml::create("/home/tdillig/Desktop/solver.glade",
"solverui"); refXml->get_widget("solverui", ui_window); kit.run(*ui_window);
        return 0;
        }
        //delete me end
        ...
        Gtk::Window* ui_window = NULL;
        Glib::RefPtr<Gnome::Glade::Xml> refXml =
                                Gnome::Glade::Xml::create("solver-ui.glade",
"solverui"); refXml->get_widget("solverui", ui_window); Gtk::Button*
solve_button = NULL; refXml->get_widget("solve", solve_button);
        assert(solve_button != NULL);
...
sigc::ptr_fun(solve_button_clicked) );
        ...
        refXml->get_widget("verify_simplification",
verify_simplification_button); assert(verify_simplification_button != NULL);
